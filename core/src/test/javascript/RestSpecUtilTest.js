/*jslint newcap: false*/
/*global $, module, equals, raises, test, RuntimeException, RestSpecUtil, MockHttpClient, XMLSerializer */
...
test("With Ampersand getSpec not wrapped throws exception", function () {
    ...
    equals(error, "Your response representation may not be well-formed.  Possible cause: an XML marker tag like < or & not wrapped in a CDATA");
});
...
test("json response is okay", function () {
    ...
    equals(restSpec.response.representation.asXml, "{\"super\":\"duper\",\"supper\":1}");
    equals(restSpec.response.representation.asText, "{\"super\":\"duper\",\"supper\":1}");
    json = restSpec.response.representation.asJson
    equals(json.super, "duper");
    equals(json.supper, 1);
...
test("text response is okay", function () {
    ...
    // then
    equals(restSpec.response.representation.asText, "whatever, it doesn't matter");
});
...
test("Without Ampersand getSpec with different representations", function () {
    ...
    equals(typeof spec.response.representation.asXml, "object");
    equals($(spec.response.representation.asXml).text(),  "Hello & Fozzy & Kermit1http://fun.html");
    equals(new XMLSerializer().serializeToString(spec.response.representation.asXml), "<results><ad><name>Hello &amp; Fozzy <![CDATA[& Kermit]]></name><id>1</id><description><![CDATA[http://fun.html]]></description></ad></results>");
});
...
test("RestSpecUtil gets a spec", function () {
    ...
    equals(spec.response.outcome, "option A");
});
...
test("RestSpecUtil replaces text in the url", function () {
    ...
    equals(spec.url, "/map/url/newText/okay");

    spec = RestSpecUtil(MockRestSpec).getSpec("respond to this", {"{replaceMe}": "secondNewText"});
    equals(spec.url, "/map/url/secondNewText/okay");
});
...
test("RestSpecUtil can replace multiple placeholders in the url", function () {
    ...
    equals(spec.url, "/map/newText/url/newText/okay/replacementX/done");
});
...
test("RestSpecUtil returns nothing when looking for a name that is not present", function () {
    ...
    equals(typeof spec, "undefined", "The type of the returned spec should be undefined (the spec wasn't found)");
...
test("RestSpecUtil replaces response.representation with the asXXX nodes MockHttpClient expects", function () {
    ...
    equals(typeof spec.response.representation, "object");
    equals(typeof spec.response.representation.asXml, "object");
    equals(typeof spec.response.representation.asJson, "string");
    equals(typeof spec.response.representation.asText, "string");

    equals(new XMLSerializer().serializeToString(spec.response.representation.asXml), "<text>where am I?</text>");
    equals(spec.response.representation.asJson, "<text>where am I?</text>");
    equals(spec.response.representation.asText, "<text>where am I?</text>");
...
test("RestSpec retrieves a file when asked nicely", function () {
    ...
    equals(spec.request.representation.asText, expectedRequestRepresentation, 'request representation gives ref content');
    equals(spec.response.representation.asText, expectedResponseRepresentation, 'response representation gives ref content');
});
...
test("RestSpecUtil resolves file-refs relative to the pathPrefix option if present", function(){
    ...
    equals(retrieved, "<message>these are the bunnies you were looking for</message>");
